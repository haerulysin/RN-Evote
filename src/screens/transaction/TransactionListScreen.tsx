...
const TransactionListScreen: FC<Props> = ({ route, navigation }: Props) => {
  ...
  const getTxList = () => {
    AsyncStorage.getItem(StorageKey.TXList)
      .then((r) => {
        if (r?.length!>=0) {
          setTXIdList(JSON.parse(r as string));
        }
      })
  ...
  const getTxDataList = () => {
    TXIdList.map((txId: string) => {
      GetTXData(txId).then((r) => {
        setTXData((prev) => ({ ...prev, [txId]: r.data }));
      });
    });
  };
  ...
  return (
    <View className="mt-3">
      ...
      >
        {/* {Object.entries(TXData).length <= 0 && <Text>No Data</Text>} */}
